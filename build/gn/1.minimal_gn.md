...
一些测试需要在运行时加载数据，或者一些目标的构建仅需要某些依赖存在即可，而不被任何构建步骤所需要。
...
在本例中导入 `buid.gn` 将在当前文件中引入两个变量:一个可能被用户覆盖，另一个是常量。
...
您可以使用 `gn path` 来查找目标之间的依赖路径。您可以回答为什么某些东西依赖于其他东西，或者公共依赖路径在哪里被破坏。
...
- current_toolchain 告诉您当前执行环境的工具链。

- host_toolchain 设置为在编译计算机上运行的适当工具链。

- default_toolchain 是“主要输出”，是GYP中的目标。
